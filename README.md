# 2022-12-12
swagger library를 이용하여 <br>
api 문서화 작업을 시작하고 있다.<br> 
* 운영 배포 완료 <br>
* 할 일 : (parameter , method 한글화 작업하기)<br>
포트폴리오는 마무리가 되어서 swagger를 이용하여 작업을 해야겠다.<br>
예상 소요일 : 1주~2주<br>

---------------------------------------------------------
# 2022-12-11
현재 heroku -> railway 라는 배포 서버로 이동을 하였다.<br>
보니깐 500시간이 무료고 그다음 사용한 만큼 나가는 서버인거 같은데<br>
일단 이 배포 서버를 사용해야겠다.<br>
...
헤로쿠 정책 변경으로 다른 무료 배포 사이트를 찾아봐야겠다.<br>
현재 koyeb를 사용해 볼 생각으로 회원가입을 하고 대기 중이다.<br>
다른 배포 사이트를 몇개 더 알아보고 찾게되면 다시 배포해야겠다.<br>
...
이정도의 라이브러리를 사용하였다.<br>


 # 2022/11/07
 일단 내가 생각한 부분의 기능 및 프로젝트는 완료 된거 같다...<br>
 이제 리팩토링을 할 수 있는 부분은 리팩토링을 하면서 추후 개발할 사항을 11월 말 까지 개발을 해볼 예정이다.<
 이미지 부분 수정 완료<br>
 관리자 페이지 유저 리스트 시 로그인 타입 확인 텍스트 추가 <br>
 추후 개발 할 사항 :<br>
 1.비밀번호 찾기(미정)<br>
 2.배치 프로그램을 이용한 관리자 페이지에서 엑셀 다운로드 하기 (배치 공부 중)<br>
